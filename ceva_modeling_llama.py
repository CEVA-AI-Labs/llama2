...
from liteml.ailabs_qat.layers.liteml_layers_quant import compFlowSoftmax, compFlowLinear
...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        emb = torch.cat((freqs, -freqs), dim=-1)  # Multiplied right half by -1 instead of the negation in the rotate function
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        #self.q_proj = compFlowLinear(self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if attention_mask is not None:
            ...
            combined_attention_mask = torch.clip(combined_attention_mask, torch.finfo(inputs_embeds.dtype).min)
